...
struct AppListView: View {
    ...
    var body: some View {
        List(apps) { app in
            ...
            System(.iOS, content:
                ^NavigationLink(
                    destination: ReleaseListView(app: app, releases: self.releases[app] ?? [], downloadingReleases: self.downloadingReleases, download: self.download)
                        .onAppear(perform: ^self.appSelected(app)
                    ),
...
struct AppListView_Previews: PreviewProvider {
    static var previews: some View {
        let apps = (0...40).map { _ in
            App(id: UUID(), description: nil, releaseType: nil, iconSource: nil, displayName: "ABC", name: "abc", os: .iOS, iconUrl: nil, owner: Owner(
                id: UUID(),
                avatarUrl: nil,
                name: "mkj",
                ...
                email: nil,
                type: .user
            ), appSecret: "somesecret", azureSubscription: nil, platform: .apple, origin: .appcenter, createdAt: Date(), updatedAt: Date(), memberPermissions: nil)
        }
